...
impl<'a> RustGenerator<'a> {
    ...
    pub fn new(compiled: Compiled<'a>, config: Config) -> Result<Self, Error<'a>> {
        let mut forced_updates = HashSet::new();
        for t in compiled.complex_types.values() {
            if t.generates_update() {
                let mut base = t.base;
                while let Some(b) = base {
                    if !forced_updates.insert(b) {
                        break;
                    }
                    base = compiled.complex_types.get(&b).and_then(|t| t.base);
                }
            }
        }
        for t in compiled.entity_types.values() {
            if t.generates_update() {
                ...
                while let Some(b) = base {
                    ...
                    base = compiled.entity_types.get(&b).and_then(|t| t.base);
                }
            }
        }
